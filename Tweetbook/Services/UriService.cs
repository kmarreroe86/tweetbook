...
namespace Tweetbook.Services
{
    public class UriService : IUriService
    {
        ...
        public Uri GetAllPost(PaginationQuery paginationQuery = null)
        {
            var uri = new Uri(_baseUri + ApiRoutes.Posts.GetAll);

            if (paginationQuery == null) return uri;

            var modifiedUri = QueryHelpers.AddQueryString(uri.ToString(), "pageNumber", paginationQuery.PageNumber.ToString());
